...
#' Analyze planning versus execution
#' @description return Q3 compare to target 
performance_data <- read.xlsx(
  file = file.path("instruction","KQKD_2020.Q3_20201110.xls"),
  sheetIndex = 1
) %>% 
  tidytable::slice.(-c(1,2,3,4,5,6))

#' Raw
raw <- readxl::read_xlsx(
  path = file.path(control$folder, "data", "earning_raw_20206M.xlsx"),
  sheet = 1, range = NULL, col_names = TRUE,
  col_types = NULL, na = "", trim_ws = TRUE, skip = 0,
  n_max = Inf, guess_max = 1000, .name_repair = "unique"
) %>%
  tidytable::mutate.(index = 1:nrow(.)) %>%
  tidytable::mutate.(earning_classification_code = tidytable::case.(
    .by = index,
    all(earning_value > 0, earning_compare_same_period > 0), "Excellence",
    all(earning_value > 0, earning_compare_same_period < 0), "Moderate",
    default = "Bad"
  )) %>%
...
earning_exchange <- raw %>%
  tidytable::arrange.(-earning_value) %>%
  tidytable::summarise.(
    .by = primary_exchange_code,
    number_consituent = n.(),
...
earning_classification <- raw %>%
  tidytable::arrange.(-earning_value) %>%
  tidytable::summarise.(
    .by = c(primary_exchange_code, earning_classification_code),
    number_consituent = n.(),
    earning_value = sum(earning_value, na.rm = TRUE),
...
earning_classification_validate <- raw %>%
  tidytable::arrange.(-earning_value) %>%
  tidytable::summarise.(
    .by = c(primary_exchange_code, earning_classification_code, fundamental_status),
    number_consituent = n.(),
    earning_value = sum(earning_value, na.rm = TRUE),
    top_10_company = paste(purrr::keep(
      purrr::map(1:10, ~ dplyr::nth(x = ticker, n = .)),
      ~ !is.na(.)
    ), collapse = ", ")
  )

#' GICS
vietnam_gics_classification <- readxl::read_xlsx(
  path = here::here("manager", "instruction", "earning_analysis", "vietnam_bloomberg_gics.xlsx"),
...
raw <- raw %>%
  dplyr::left_join(dplyr::select(
    vietnam_gics_classification, ticker,
    dplyr::ends_with("_name")
  ), by = "ticker")
...
detect_extreme_raw <- raw %>%
  ...
  dplyr::summarise(
    pe_industry_average = mean(current_pe_ratio, na.rm = TRUE),
    ...
    pe_industry_range = paste(min(current_pe_ratio, na.rm = TRUE),
      max(current_pe_ratio, na.rm = TRUE),
      sep = " - "
    )
  ) %>%
  dplyr::ungroup()
...
normal_raw <- raw %>%
  ...
  dplyr::mutate(is_extreme = dplyr::case_when(
    current_pe_ratio > pe_industry_cap | current_pe_ratio < pe_industry_floor ~ "Extreme",
    TRUE ~ "Normal"
  )) %>%
...
ticker_valuation <- raw %>%
  ...
  dplyr::mutate(valuation_status = dplyr::case_when(
    valuation_percentage > 0 ~ "Premium",
    TRUE ~ "Discount"
  ))
